/* autogenerated by Processing revision 1307 on 2025-09-10 */
...
public void setup() {
  ...
  mono = createFont("Courier", PApplet.parseInt(charSize));
  textFont(mono);
  ...
  cp5.addSlider("contrastAdjust").setPosition(20, 220).setRange(0.5f, 3).setValue(contrastAdjust);
...
public void spriteCols(float val) { spriteCols = max(4, PApplet.parseInt(floor(val))); regenerateFrames(); }
public void spriteRows(float val) { spriteRows = max(2, PApplet.parseInt(floor(val))); regenerateFrames(); }
public void asciiColorPicker(int c) { asciiColor = c; }
  ...
  for (int i = 0; i < frames.length; i++) {
    ...
    for (int y = 0; y < img.height; y++) {
      for (int x = 0; x < img.width; x++) {
        ...
        sb.append(letters.charAt(PApplet.parseInt(map(b, 0, 255, 0, letters.length() - 1))));
      }
      sb.append("\n");
    }
...
public void startGifExport() {
  ...
  // Crear el GIF
  gif = new GifMaker(this, gifPath, PApplet.parseInt(1000.0f / frameSpeed));
...
public PImage asciiFrame(PImage img) {
  ...
  pg.fill(asciiColor);
  float xStep = width / PApplet.parseFloat(img.width);
  float yStep = height / PApplet.parseFloat(img.height);
  img.loadPixels();
  for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) {
      int idx = y * img.width + x;
      ...
      if (threshold > 0) b = (b > threshold) ? 255 : 0;
      if (negativeMode) b = 255 - b;
      pg.text(letters.charAt(PApplet.parseInt(map(b, 0, 255, 0, letters.length() - 1))), x * xStep, y * yStep);
    }
  }
  pg.endDraw();
  return pg.get();
...
public void draw() {
  ...
  for (int y = 0; y < img.height; y++) {
    for (int x = 0; x < img.width; x++) {
      ...
      if (negativeMode) b = 255 - b;
      fill(asciiColor);
      text(letters.charAt(PApplet.parseInt(map(b, 0, 255, 0, letters.length() - 1))), x * xStep, y * yStep);
    }
  }
  if (frameCount % PApplet.parseInt(60.0f / frameSpeed) == 0) {
...
public File selectFolder() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "ascii_art" };
    if (passedArgs != null) {
